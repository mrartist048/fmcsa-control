...
// ====== PROFESSIONAL DARK SIDEBAR UI ARCHITECTURE ======
function injectDarkSidebarUI() {
    ...
    contentWrapper.style.cssText = "margin-left: 260px; padding: 20px; box-sizing: border-box; width: calc(100% - 260px); min-height: 100vh; background: #f8fafc;";
    ...
    sidebar.innerHTML = `
        ...
        <!-- Agent Profile Box -->
        <div style="padding: 15px 18px; background: #1e293b; margin: 12px 12px 0 12px; border-radius: 8px; border: 1px solid #334155;">
            <div style="font-size: 10px; color: #94a3b8; text-transform: uppercase; font-weight: bold; margin-bottom: 4px;">Active Dispatcher</div>
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <span id="dlSidebarAgentName" style="font-size: 13px; font-weight: bold; color: #f8fafc; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 140px;">${dispatcherNickname}</span>
                <button onclick="changeDispatcherName()" title="Edit Name" style="background: #334155; border: none; color: #cbd5e1; padding: 3px 6px; border-radius: 4px; font-size: 10px; cursor: pointer; font-weight: bold;">✏️ Edit</button>
            </div>
        </div>

        <!-- Navigation Menu -->
        <div style="flex: 1; overflow-y: auto; padding: 15px 12px; display: flex; flex-direction: column; gap: 6px;">
            ...
            <div style="font-size: 10px; color: #64748b; font-weight: bold; text-transform: uppercase; padding: 12px 8px 4px 8px; margin-top: 6px;">Email Setup Template</div>
            <div style="background: #1e293b; padding: 10px; border-radius: 6px; border: 1px solid #334155; display: flex; flex-direction: column; gap: 8px;">
                <input type="text" id="sidebarPropSubject" value="${savedSubject}" placeholder="Email Subject" style="width: 100%; background: #0f172a; border: 1px solid #475569; color: white; padding: 6px 8px; font-size: 11px; border-radius: 4px; box-sizing: border-box;">
                <textarea id="sidebarPropBody" placeholder="Email Body ({company})" style="width: 100%; height: 75px; background: #0f172a; border: 1px solid #475569; color: white; padding: 6px 8px; font-size: 11px; border-radius: 4px; box-sizing: border-box; resize: vertical;">${savedBody}</textarea>
                <button onclick="saveSidebarProposalSettings()" style="background: #0284c7; color: white; border: none; padding: 6px; font-size: 11px; font-weight: bold; border-radius: 4px; cursor: pointer; width: 100%;">💾 Save Template</button>
            </div>
        </div>
...
window.changeDispatcherName = function() {
    ...
    if (newName && newName.trim() !== "") {
        ...
        localStorage.setItem(`dl_nick_${currentClient}`, dispatcherNickname);
        let label = document.getElementById('dlSidebarAgentName');
...
window.saveSidebarProposalSettings = function() {
    let subjVal = document.getElementById('sidebarPropSubject')?.value || "";
    let bodyVal = document.getElementById('sidebarPropBody')?.value || "";
