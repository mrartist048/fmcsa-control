...
// ====== BULLETPROOF INDEPENDENT TAB INSTANCE ID (FIXED FOR DUPLICATE TABS) ======
if (!window.name || !window.name.startsWith("dl_tab_inst_")) {
    window.name = "dl_tab_inst_" + Math.random().toString(36).substr(2, 9) + "_" + Date.now() + "_" + Math.floor(Math.random() * 100000);
}
...
async function initializeAccessControl() {
    ...
    setInterval(checkGlobalSessions, 6000);
}
...
async function checkGlobalSessions() {
    ...
    try {
        ...
        const offlineThreshold = 14000; // 14 seconds
...
window.stopScraping = function() {
    ...
    if (statusBox) {
        statusBox.innerHTML = "<strong>⏸️ Processing Paused Safely. Click Start to resume.</strong>";
    }
}

// ====== HIGH-SPEED ROBUST SCRAPER WITH PROXIED/DIRECT FALLBACKS ======
    ...
    while (attempt < maxRetries) {
        try {
            ...
            for (let i = 0; i < cells.length; i++) {
                ...
                if (text.startsWith("Physical Address:") || text.startsWith("Address:")) {
            ...
            // Extract email & vehicle info directly from snapshot text or fallback regex search
            ...
            // Quick SMS check if USDOT available
            if (record.usdot !== 'N/A') {
                try {
                    ...
                    if (smsRes.ok) {
                        ...
                        smsEl.querySelectorAll('table tr').forEach(row => {
                            ...
                            if (cols.length >= 4) {
                                ...
                                if (count > 0 && ["Straight Trucks", "Truck Tractors", "Trailers"].includes(vType)) {
                                    vehicleList.push(`${vType} ${count}`);
                                }
                            }
                        });
...
window.startScraping = async function(overrideStart = null, overrideEnd = null) {
    ...
    if (isNaN(start) || isNaN(end) || start > end) {
        alert("Please enter a valid MC range.");
        return;
    }
    ...
    for (let mc = start; mc <= end; mc++) {
        ...
        if (result.status === "success" && result.data) {
            ...
            let tableBody = document.getElementById('resultsTable') || document.querySelector('table');
            if (tableBody && tableBody.tagName === 'TABLE') {
        ...
        await new Promise(r => setTimeout(r, 100)); // Optimized speed
    }
